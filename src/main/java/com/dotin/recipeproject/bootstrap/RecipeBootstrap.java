package com.dotin.recipeproject.bootstrap;

import ch.qos.logback.core.spi.ContextAware;
import com.dotin.recipeproject.enumerations.Difficulty;
import com.dotin.recipeproject.model.*;
import com.dotin.recipeproject.repository.CategoryRepository;
import com.dotin.recipeproject.repository.RecipeRepository;
...
public class RecipeBootstrap implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public RecipeBootstrap(UnitRepository unitRepository, CategoryRepository categoryRepository, RecipeRepository recipeRepository) {
    ...
    private List<Recipe> getRecipes() {
        ...
        //get UOMs
        Optional<UnitOfMeasure> eachUomOptional = unitRepository.findByDescription("Each");

        if(!eachUomOptional.isPresent()){
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UnitOfMeasure> tableSpoonUomOptional = unitRepository.findByDescription("Tablespoon");

        if(!tableSpoonUomOptional.isPresent()){
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UnitOfMeasure> teaSpoonUomOptional = unitRepository.findByDescription("Teaspoon");

        if(!teaSpoonUomOptional.isPresent()){
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UnitOfMeasure> dashUomOptional = unitRepository.findByDescription("Dash");

        if(!dashUomOptional.isPresent()){
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UnitOfMeasure> pintUomOptional = unitRepository.findByDescription("Pint");

        if(!pintUomOptional.isPresent()){
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UnitOfMeasure> cupsUomOptional = unitRepository.findByDescription("Cup");
